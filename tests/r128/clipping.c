...
test_src_clipping_latches(ati_device_t *dev)
{
    ...
    wr_r128_dp_gui_master_cntl(dev, r128_dp_gui_master_cntl & ~0x4);

    // src_sc_bottom and src_sc_right should latch default
    ASSERT_EQ(rd_r128_dp_gui_master_cntl(dev), r128_dp_gui_master_cntl & ~0x4);
    ASSERT_EQ(rd_default_sc_bottom_right(dev), 0x0aaa0bbb);
...
test_dst_clipping_latches(ati_device_t *dev)
{
    ...
    r128_dp_gui_master_cntl = rd_r128_dp_gui_master_cntl(dev);
    wr_r128_dp_gui_master_cntl(dev, r128_dp_gui_master_cntl & ~0x8);

    // sc_bottom and src_right should latch default
    // sc_top and sc_left are set to the origin
    ASSERT_EQ(rd_r128_dp_gui_master_cntl(dev), r128_dp_gui_master_cntl & ~0x8);
    ASSERT_EQ(rd_default_sc_bottom_right(dev), 0x0aaa0bbb);
...
void
register_r128_clipping_tests(void)
{
    REGISTER_TEST_FOR(test_src_clipping_latches, "SRC clipping latches", CHIP_R128);
    REGISTER_TEST_FOR(test_dst_clipping_latches, "DST clipping latches", CHIP_R128);
}
